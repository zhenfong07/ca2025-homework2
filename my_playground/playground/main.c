...
/* ================================================
 * Khai báo từ hanoi.S // <<< MỚI
 * ================================================ */
extern void hanoi_asm(void); // Báo cho C biết có hàm hanoi_asm


/* ================================================
 * RSQRT + distance 3D (Giữ nguyên)
 * ================================================ */
...
uint32_t fast_rsqrt(uint32_t x) {
    if (x<=1) return (x==0) ? 0xFFFFFFFFu : 65536; 
 ...
 * ================================================ */
int main(void) {
    // Biến đếm dùng chung cho cả 2 bài test
    ...
    TEST_LOGGER("=== Start Test Suite ===\n");

    // ---------- BÀI TEST 1: RSQRT (Problem C) ----------
    ...
    // ---------- BÀI TEST 2: HANOI (Problem B) ---------- // <<< MỚI
    TEST_LOGGER("\n\n--- Problem B: Tower of Hanoi ---\n");

    // Bắt đầu đo riêng cho Hanoi
    start_cycles = get_cycles();
    start_instret = get_instret();
    start_ticks = getticks();

    hanoi_asm(); // <<< GỌI HÀM ASSEMBLY CỦA BẠN

    // Kết thúc đo cho Hanoi
    end_cycles = get_cycles();
    end_instret = get_instret();
    end_ticks = getticks();

    // Tính toán (ta dùng lại biến _c)
    cycles_c = end_cycles - start_cycles;
    instret_c = end_instret - start_instret;
    ticks_c = end_ticks - start_ticks;

    // In kết quả cho Hanoi
